...
export function useModel({
  ...
  // Get available models from the model store
  const { models } = useModelStore()

  // Calculate the effective model based on priority: chat model > first favorite model > first available model > hardcoded default
  const getEffectiveModel = useCallback(() => {
    const firstFavoriteModel = user?.favorite_models?.[0]
    const firstAvailableModel = models.length > 0 ? models[0].id : undefined

    return currentChat?.model || firstFavoriteModel || firstAvailableModel || MODEL_DEFAULT
