...
import { WebSearchAnimation } from "./web-search-animation"
...
function SingleToolCard({
  ...
  // Special rendering for web_search when loading
  if (isWebSearch && isLoading) {
    console.log('[ToolInvocation] Showing web search animation')
    return (
      <div
        className={cn(
          "border-border flex flex-col gap-0 overflow-hidden rounded-md border bg-blue-50/50 dark:bg-blue-950/30",
          className
        )}
      >
        <WebSearchAnimation />
  ...
  // Log for debugging
  if (isWebSearch) {
    console.log('[ToolInvocation] Web search tool - Loading:', isLoading, 'Completed:', isCompleted)
  }
