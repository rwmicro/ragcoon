...
export function LayoutSettings() {
  const { preferences, setNavigationPosition } = useUserPreferences()

  const handleNavigationPositionChange = (position: NavigationPosition) => {
  ...
  return (
    <div className="space-y-6">
      {/* Navigation Position */}
      <div>
        <h3 className="mb-3 text-sm font-medium">Navigation Position</h3>
        <p className="text-xs text-muted-foreground mb-3">
          Choose where to display navigation controls
        </p>
        <div className="grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => handleNavigationPositionChange("sidebar")}
            className={cn(
              "rounded-lg border p-3 text-left transition-colors",
              preferences.navigationPosition === "sidebar"
                ? "border-primary ring-primary/30 ring-2"
                : "border-border hover:bg-muted/50"
            )}
          >
            <div className="flex flex-col items-center gap-2">
              <LayoutSidebar className="h-24 w-full" />
              <span className="text-xs font-medium">Sidebar Left</span>
            </div>
          </button>

          <button
            type="button"
            onClick={() => handleNavigationPositionChange("header")}
            className={cn(
              "rounded-lg border p-3 text-left transition-colors",
              preferences.navigationPosition === "header"
                ? "border-primary ring-primary/30 ring-2"
                : "border-border hover:bg-muted/50"
            )}
          >
            <div className="flex flex-col items-center gap-2">
              <LayoutFullscreen className="h-24 w-full" />
              <span className="text-xs font-medium">Header Top</span>
            </div>
          </button>
        </div>
      </div>
