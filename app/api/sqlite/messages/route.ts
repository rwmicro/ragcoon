  ...
  try {
    ...
    switch (action) {
      case 'get': {
        ...
        // Convert database format to AI SDK format
        const formattedMessages = messages.map(msg => {
          // DO NOT load parts - they cause validation errors
          // We only use experimental_attachments for RAG sources
          // Parts are removed to avoid AI SDK validation issues
          ...
          if (msg.experimental_attachments) {
            try {
              ...
              console.log('[SQLite] Loaded attachments for message:', msg.id, experimentalAttachments?.length || 0)
            } catch (e) {
          ...
          return {
            ...
            // DO NOT include parts - causes validation errors
          }
        })
      ...
      case 'create': {
        // Create a single message (use INSERT OR IGNORE to avoid duplicate errors)
      ...
      case 'createMany': {
        // Create multiple messages (use INSERT OR IGNORE to avoid duplicate errors)
