...
- **Multi-provider LLM** — Ollama (local) and Hugging Face Inference API models out of the box
...
The app will be available at <http://localhost:3000>. SQLite data is persisted in a named Docker volume (`ragcoon_data`). By default the container expects Ollama and the RAG backend to run on the host machine; override `OLLAMA_BASE_URL` and `NEXT_PUBLIC_RAG_API_URL` in `.env` if they live elsewhere.
...
The app starts at <http://localhost:3000>. Make sure Ollama and the RAG backend are running before opening the interface.
